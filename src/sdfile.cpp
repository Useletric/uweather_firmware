...
void getID()
{
  ...
  uint64_t resultado = struct_systemConfig.chipId * struct_systemConfig.multiplicador;
   char idString[17]; // Assumindo que a string hexadecimal terá até 16 caracteres (mais um para o terminador nulo)

    // Formatar o resultado hexadecimal como uma string
    sprintf(idString, "%llX", resultado);

    // Salvar na variável struct_systemConfig.idStation
    struct_systemConfig.idStation =  idString;
    Serial.print("id: ");
    Serial.println(resultado, HEX);
    Serial.println(struct_systemConfig.idStation);
}
...
void salvarLeitura()
{
  ...
  if (SD.exists("/leituras.txt"))
  {
    SD.remove("/leituras.txt");
  }

  store_sd("/leituras.txt", doc);
}
